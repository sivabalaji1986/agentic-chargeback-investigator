...
## 2026-07-23 — Lease-Based Agent Registry: Prompt 5 batch

**Files:** `agent-registry/` (new: `config.py`, `clock.py`, `models.py`, `logging.py`, `repository.py`, `service.py`, `lease_manager.py`, `api.py`, `main.py` + tests), `pyproject.toml` (root — mypy exclude list narrowed, `agent-registry` dependency added), `Makefile` (`registry-test`/`registry-run` targets), `README.md`
**What:** Built `agent-registry`, a standalone FastAPI service providing lease-based runtime capability discovery, entirely in-memory (no database). `RegistryService` sits over an `asyncio.Lock`-guarded `AgentRepository` and an injectable `Clock` (`SystemClock` for the real server, `FakeClock` for deterministic tests — lease-expiry tests advance simulated time instead of sleeping). Endpoints: `POST /agents` (idempotent upsert by `agent_id` — re-registering an active agent is a refresh, not an error, returning `200` vs `201` for new), `POST /agents/{agent_id}/renew`, `DELETE /agents/{agent_id}` (both `404` on unknown), `GET /agents`, `GET /agents/capabilities`, `GET /agents/discover?capability=...` (an unmatched or unknown-but-valid capability returns an empty list, not an error), and `GET /health`. Capabilities reuse the existing `chargeback_contracts.skills.SkillId` enum throughout — no capability constant is duplicated. A background `LeaseManager` task, wired through FastAPI's `lifespan`, sweeps expired leases on a configurable interval; expiry and deregistration both fully remove the record (one code path), so a lapsed `agent_id` can freely re-register. Logging follows `dispute-mcp-server`'s proven explicit-handler pattern (never `logging.basicConfig`), applying a lesson from Prompt 4. `make verify` initially failed after all 10 build tasks were individually reviewed and approved: `agent-registry`'s new test files (`test_service.py`, `test_api.py`, `test_logging.py`) collided under mypy with identically-named files already in `transaction-agent`/`dispute-mcp-server`, since no `tests/` directory in this repo has an `__init__.py`. Diagnosed the root cause, empirically tested and rejected adding `tests/__init__.py` (verified it only pushes the collision up one level, to the bare name `"tests"` itself), and fixed it by renaming only the 3 new, colliding `agent-registry` test files (`test_registry_service.py`/`test_registry_api.py`/`test_registry_logging.py`) — a pure rename, zero content change, touching neither `transaction-agent` nor `dispute-mcp-server`. Also fixed a smaller, unrelated latent gap found along the way: `config.py` passed `ruff check` but not `ruff format --check` (a multi-line call that fits under the 100-column limit unwrapped). After these fixes, `make verify` passes fully clean: ruff format-check and lint clean, strict mypy clean across 83 source files, full pytest suite at 213 passed/1 skipped, frontend build clean. Confirmed `transaction-agent/src`, `investigator-ui/src`, and `dispute-mcp-server/src` have zero diff against the batch's start commit — Prompt 4's vertical slice is untouched. No Orchestrator, A2A, additional specialist agents, RAG, or recommendation rules were implemented, and `agent-registry` is not yet integrated with `transaction-agent` — all explicitly deferred to a later prompt.
**Why:** Prove runtime capability discovery works as its own standalone, testable service — lease register/renew/expire/deregister and capability-based discovery — before any agent actually depends on it; deferring the Transaction-Agent integration keeps this batch focused and reviewable on its own, consistent with how each earlier prompt in this build has stood alone as working, testable software.

## 2026-07-22 — Vertical Slice (End-to-End Spike): Prompt 4 batch

**Files:** `transaction-agent/` (new package: `config.py`, `llm_client.py`, `mcp_client.py`, `service.py`, `api.py`, `main.py` + tests), `investigator-ui/src/` (`types.ts`, `lib/agent.ts`, `components/{EventStream,InvestigationTimeline,RecommendationCard,DecisionCard}.tsx`, `App.tsx`), `.env.example`, `.gitignore`, `pyproject.toml`, `README.md`
**What:** Built the first end-to-end vertical slice proving the architecture: a temporary intake harness calls the new `transaction-agent` (FastAPI), which fetches seeded case/transaction data from `dispute-mcp-server` via an in-process `fastmcp.Client`, streams the 5 required AG-UI milestones (Investigation Started, MCP Lookup, Transaction Loaded, Recommendation Ready, Investigation Completed) over SSE using existing Prompt 2 `chargeback_contracts.agui` payloads (no new contracts needed), and returns a fixed mock recommendation (`request_more_evidence`) plus an LLM-generated explanation (real host Ollama, `qwen3.5:latest`, `think=false`) wrapped in an A2UI v0.9 `DecisionCard` envelope. `investigator-ui` gained four reusable components (`InvestigationTimeline`, `EventStream`, `RecommendationCard`, `DecisionCard`) wired together in `App.tsx` via `@ag-ui/client`'s `HttpAgent`, and an investigator's Approve/Reject/Request-More-Evidence action round-trips to a logging `/actions/decision` endpoint. Real, non-mocked verification throughout: a live-Ollama test confirmed no `<think>` leakage and valid structured JSON; a real Playwright browser session drove the full flow end-to-end (screenshots taken, not committed) and caught a genuine defect — the backend never called `logging.basicConfig`, so the `investigator_decision` audit log line was silently dropped in the real running process despite passing under pytest's `caplog` — fixed with a one-line `logging.basicConfig` call in `create_app()`. Also fixed two other latent, previously-undiscovered bugs found while doing this batch's work: an unanchored `.gitignore` `lib/` rule that silently matched the new `investigator-ui/src/lib/` directory (anchored to `/lib/`, `/lib64/`), and a stale `qwen3.5:9b` model-tag reference in README's Prerequisites section that didn't match the tag actually installed on the target Ollama host (already corrected in `.env.example` earlier in this same batch, but missed in README at the time). `make verify` passes clean, including the real-Ollama test actually executing (not skipping) and the frontend production build. No Agent Registry, Orchestrator, additional specialist agents, RAG, or deterministic recommendation logic were implemented — all explicitly deferred to later prompts.
**Why:** Prove the full architecture (intake → Transaction Agent → MCP → AG-UI → investigator-ui → A2UI decision → investigator action → backend) works end-to-end before building out the remaining specialist agents, the Orchestrator, and deterministic recommendation logic in later prompts; the intake harness is deliberately temporary spike wiring (replaced by the Orchestrator in a later prompt), while the Transaction Agent's core logic, the AG-UI integration, the UI components, and the action endpoint are built to be reused, not thrown away.
